...
class OptInPage implements \ShopMagicCartVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function __construct($plugin_file, $plugin_slug, $shop = null)
    {
        ...
        $this->shop = $shop ?? new \ShopMagicCartVendor\WPDesk\Tracker\Shop('');
    }
    ...
    public function output()
    {
        ...
        if (isset($_GET['shop_url'])) {
            $shop = new \ShopMagicCartVendor\WPDesk\Tracker\Shop(\sanitize_text_field(\wp_unslash($_GET['shop_url'])));
        } else {
            $shop = $this->shop;
        }
        ...
        $renderer = new \ShopMagicCartVendor\WPDesk\View\Renderer\SimplePhpRenderer(new \ShopMagicCartVendor\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
