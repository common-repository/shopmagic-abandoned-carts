    ...
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        public function send_payload(array $payload)
        {
            if (\class_exists('ShopMagicCartVendor\\WPDesk_Logger_Factory')) {
                \ShopMagicCartVendor\WPDesk_Logger_Factory::log_message('Sender payload: ' . \json_encode($payload), self::LOGGER_SOURCE, \ShopMagicCartVendor\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \ShopMagicCartVendor\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \ShopMagicCartVendor\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\ShopMagicCartVendor\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \ShopMagicCartVendor\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
