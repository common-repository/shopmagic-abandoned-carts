 ...
 */
class WooTemplateResolver implements \ShopMagicCartVendor\WPDesk\View\Resolver\Resolver
{
    /** @var string */
    private $base_path;
    ...
    public function resolve($name, \ShopMagicCartVendor\WPDesk\View\Renderer\Renderer $renderer = null) : string
    {
        $template = \wc_locate_template($name, '', $this->base_path);
        if ($template === '') {
            throw new \ShopMagicCartVendor\WPDesk\View\Resolver\Exception\CanNotResolve("Cannot resolve template {$name}");
        }
        return $template;
    }
}
